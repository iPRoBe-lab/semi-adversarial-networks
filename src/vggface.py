...
class VGGface(nn.Module):
    def __init__(self, num_classes=2):
        ...
        if use_cuda:
            self.gray_correction = torch.autograd.Variable(vgg_gray_corr_ptch, requires_grad=False).cuda()
        else:
            self.gray_correction = torch.autograd.Variable(vgg_gray_corr_ptch, requires_grad=False)
